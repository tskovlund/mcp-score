"""Where MuseScore's executable is.

The search order is the ``MCP_SCORE_MUSESCORE_PATH`` environment variable,
the well-known executable names on PATH, then the platform's default
install location (macOS app bundle, Windows Program Files, Linux Flatpak).
A command is a list because the Flatpak launcher is a multi-word command.
"""
